...
def _parse_tx_hash(response) -> str:
    """Extract tx signature from x-payment-response header."""
...
def pay(vendor_id: str, amount: float) -> dict:
    ...
    Falls back to mock on any failure so the badge -> 🟡.
    """
    if not _PRIVATE_KEY:
        return _MOCK_RESULT
    ...
    try:
        ...
        # The TS demo server acts as the vendor API.
        # /premium costs $0.0001 USDC — sufficient for demo proof.
